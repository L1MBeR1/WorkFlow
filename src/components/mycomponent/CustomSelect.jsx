        ...
        <div className="custom-select" onClick={handleToggleSelect} tabIndex="0" onBlur={closeSelector}>
            ...
            {props.type === 'parameters' && (
                  props.options.map((item, index) => (
                    <div key={index} className='custom-select-item' onClick={() => handleSelect(item.name)}> {item.name}</div>
                  ))
                )}
            {props.type === 'services' && (
                  props.options.map((item, index) => (
                    <div key={index} className='custom-select-item' onClick={() => handleSelect(item.Название)}> {item.Название}</div>
                  ))
                )}
            {props.type === 'uri' && (
                  props.options.map((item, index) => (
                    <div key={index} className='custom-select-item' onClick={() => handleSelect(item.uri)}> {item.uri
                    }</div>
                  ))
                )}
                {/* props.options.map((item, index) => (
